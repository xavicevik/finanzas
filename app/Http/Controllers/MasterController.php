...
class MasterController extends Controller
{
    ...
    public function corporacionesIndex(Request $request)
    {
        ...
        if ($buscar == ''){
            $corporaciones = Corporacion::orderBy($sortBy, $sortOrder)->where('estado', 1)
                ->paginate(self::canPorPagina);
        } else {
            $corporaciones = Corporacion::orderBy($sortBy, $sortOrder)
                ->where('nombre', 'like', '%'. $buscar . '%')->where('estado', 1)
                ->paginate(self::canPorPagina);
        }
    ...
    public function corporacionesregionales(Request $request)
    {
        $corporacion =  Corporacion::where('estado', 1)->get();
